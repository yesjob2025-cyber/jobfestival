...
import { isFestAdmin } from "@/lib/session";
import {
  listAllApplications,
  listAllCheckins,
  listAllInterviews,
...
import { FEST, courseName } from "@/lib/config";
...
export default async function FestAdminPage() {
  if (!(await isFestAdmin())) redirect("/admin/login");
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {["visitors", "applications", "checkins", "surveys"].map((t) => (
            <a key={t} href={`/api/export?type=${t}`} className="fest-btn-outline fest-btn-sm">
              {t} CSV
            </a>
          ))}
        </div>
